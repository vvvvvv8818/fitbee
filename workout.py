...
img = cv2.imread("workout3.jpg")
img2 = cv2.imread("workout4.jpg")
		...
		#frmask = mog2.apply(frame,learningRate =0.01) 
			...
			for idx, centroid in enumerate(centroids):  
				...
				if area > 5000 and width>50 and height>100 :
					...
					global crop
					crop = frame4[y:y+height,x:x+width]
					#crop = cv2.resize(crop,(129,129),cv2.INTER_CUBIC)
					img= cv2.resize(img, (crop.shape[1],crop.shape[0]), interpolation=cv2.INTER_CUBIC)
					...
					#cv2.imshow("img",img)
					#similar = cv2.matchTemplate(img,crop,cv2.TM_CCOEFF_NORMED)
